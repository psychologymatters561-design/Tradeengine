...
import { mockEvents, mockStocks } from '../lib/mockData';
...
export function AITriggers() {
  ...
  const handleGenerateRationale = async (eventId: string, symbol: string, eventHeadline: string) => {
    setSelectedEventId(eventId);
    const stock = mockStocks.find(s => s.symbol === symbol);
    const currentPrice = stock ? stock.price : 1000; // fallback

    // If already generated or loading, just show/expand and don't re-fetch immediately unless desired
    if (rationaleMap[eventId]) {
      return; 
    }
      ...
      <div className="flex justify-between items-center mb-8">
         <div>
          <h2 className="text-2xl font-semibold text-slate-100 tracking-tight flex items-center">
            AI Event Triggers <Sparkles className="w-5 h-5 ml-2 text-indigo-400" />
          </h2>
          <p className="text-sm text-slate-400 mt-1">Live news and technical events monitored by AI to predict price momentum.</p>
        </div>
      </div>
        ...
        {mockEvents.map(event => {
          ...
          return (
            <div key={event.id} className={cn(
               ...
               isSelected ? "border-indigo-500/50 shadow-[0_0_15px_rgba(99,102,241,0.15)]" : "border-slate-800 hover:border-slate-700"
            )}>
              <div className="flex justify-between items-start mb-4">
                <div className="flex items-center space-x-3">
                  ...
                  <div>
                    <h3 className="font-bold text-slate-100">{event.symbol}</h3>
                ...
                <div className="flex flex-col items-end">
                   {event.impact === 'Positive' && <span className="flex items-center text-xs font-semibold text-emerald-400 bg-emerald-400/10 px-2.5 py-1 rounded-full"><TrendingUp className="w-3 h-3 mr-1" /> Bullish Trigger</span>}
                   {event.impact === 'Negative' && <span className="flex items-center text-xs font-semibold text-rose-400 bg-rose-400/10 px-2.5 py-1 rounded-full"><TrendingDown className="w-3 h-3 mr-1" /> Bearish Trigger</span>}
                </div>
              </div>
              ...
              <div className="mt-5 pt-4 border-t border-slate-800/80">
                {!rationaleInfo ? (
                  <button 
                    onClick={() => handleGenerateRationale(event.id, event.symbol, event.headline)}
